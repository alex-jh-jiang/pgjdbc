...
public class VxLogicalReplicationStatusTest {
  ...
  private VxConnection replicationConnection;
  private VxConnection sqlConnection;

  @Before
  public void setUp() throws Exception {
    //statistic available only for privileged user
    sqlConnection = VxTestUtil.openPrivilegedDB().get();
    //DriverManager.setLogWriter(new PrintWriter(System.out));
    replicationConnection = openReplicationConnection();
    VxTestUtil.createTable(sqlConnection, "test_logic_table",
        "pk serial primary key, name varchar(100)");

    VxTestUtil.recreateLogicalReplicationSlot(sqlConnection, SLOT_NAME, "test_decoding");
  }

  @After
  public void tearDown() throws Exception {
    replicationConnection.close();
    VxTestUtil.dropTable(sqlConnection, "test_logic_table");
    VxTestUtil.dropReplicationSlot(sqlConnection, SLOT_NAME);
    sqlConnection.close();
  }

  @Test()
  public void testSentLocationEqualToLastReceiveLSN() throws Exception {
    VxConnection pgConnection = (VxConnection) replicationConnection;
  ...
  private LogSequenceNumber getSentLocationOnView() throws Exception {
    return getLSNFromView((((VxConnection) sqlConnection).haveMinimumServerVersion(ServerVersion.v10)
  ...
  private LogSequenceNumber getLSNFromView(String columnName) throws Exception {
    int pid = ((VxConnection) replicationConnection).getBackendPID();
    ...
    while (true) {
      ...
      VxResultSet rs = null;
      try {
        rs = st.executeQuery("select * from pg_stat_replication where pid = " + pid).get();

        String result = null;
        if (rs.next().get()) {
          result = rs.getString(columnName).get();
        }
    ...
    try {
      try {
        rs = st.executeQuery("select "
            + (((VxConnection) sqlConnection).haveMinimumServerVersion(ServerVersion.v10)
