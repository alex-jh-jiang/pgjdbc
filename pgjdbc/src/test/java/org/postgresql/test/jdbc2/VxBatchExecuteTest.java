 ...
 * - test VxPreparedStatement as thoroughly as VxStatement
 */

/*
 * Test case for VxStatement.batchExecute()
 */
@RunWith(Parameterized.class)
public class VxBatchExecuteTest extends VxBaseTest4 {

  private boolean insertRewrite;

  public VxBatchExecuteTest(BinaryMode binaryMode, boolean insertRewrite) {
  ...
  public void testEmptyClearBatch() throws Exception {
    ...
    VxPreparedStatement ps = con.prepareStatement("SELECT ?");
    ps.clearBatch(); // No-op.
  }

  private void assertCol1HasValue(int expected) throws Exception {
    VxStatement getCol1 = con.createStatement();

    VxResultSet rs = getCol1.executeQuery("SELECT col1 FROM testbatch WHERE pk = 1").get();
    Assert.assertTrue(rs.next().get());

    int actual = rs.getInt("col1").get();

    Assert.assertEquals(expected, actual);

    Assert.assertEquals(false, rs.next().get());
  ...
  public void testExecuteEmptyPreparedBatch() throws Exception {
    ...
    try {
      ...
      int[] updateCount = ps.executeBatch().get();
  ...
  @Test
  public void testPreparedNoParameters() throws SQLException, InterruptedException, ExecutionException {
    VxPreparedStatement ps = null;
    try {
  ...
  public void testSelectInBatch() throws Exception {
    ...
    Assert.assertTrue("Second update should succeed, thus updateCount should be 1 or SUCCESS_NO_INFO"
        ...
        updateCounts[2] == 1 || updateCounts[2] == VxStatement.SUCCESS_NO_INFO);
  ...
  public void testSelectInBatchThrows() throws Exception {
    ...
    int[] updateCounts;
    try {
      updateCounts = stmt.executeBatch().get();
    ...
    boolean firstOk = updateCounts[0] == 1 || updateCounts[0] == VxStatement.SUCCESS_NO_INFO;
    boolean lastOk = updateCounts[2] == 1 || updateCounts[2] == VxStatement.SUCCESS_NO_INFO;
    ...
    Assert.assertEquals("SELECT 0/0 should be marked as VxStatement.EXECUTE_FAILED",
        VxStatement.EXECUTE_FAILED,
        updateCounts[1]);

    stmt.close();
  }

  private int getCol1Value() throws SQLException, InterruptedException, ExecutionException {
    VxStatement stmt = con.createStatement();
    try {
      VxResultSet rs = stmt.executeQuery("select col1 from testbatch where pk=1").get();
      rs.next().get();
  ...
  public void testStringAddBatchOnPreparedStatement() throws Exception {
    ...
    try {
      ...
      Assert.fail(
          "Should have thrown an exception about using the string addBatch method on a prepared VxStatement.");
  ...
  public void testPreparedStatement() throws Exception {
    ...
    // Note that the first parameter changes for every VxStatement in the
    ...
    // now test to see that we can still use the VxStatement after the execute
    pstmt.setInt(1, 3);
    pstmt.addBatch();
    assertCol1HasValue(7);
  ...
  public void testTransactionalBehaviour() throws Exception {
    ...
    // The VxStatement has been added to the batch, but it should not yet
    // have been executed.
    assertCol1HasValue(0);
  ...
  public void testBatchEscapeProcessing() throws SQLException, InterruptedException, ExecutionException {
    ...
    VxResultSet rs = stmt.executeQuery("SELECT d FROM batchescape").get();
    Assert.assertTrue(rs.next().get());
  ...
  public void testBatchWithEmbeddedNulls() throws SQLException, InterruptedException, ExecutionException {
    ...
    VxResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM batchstring").get();
    Assert.assertTrue(rs.next().get());
  ...
  public void testMixedBatch() throws SQLException, InterruptedException, ExecutionException {
    try {
      ...
      int[] batchResult = st.executeBatch().get();
   ...
   * non-NULL afterwards using VxPreparedStatement.setObject(int, Object) (i.e. no Types param or
   ...
   */
  @Test
  public void testBatchReturningMixedNulls() throws SQLException, InterruptedException, ExecutionException {
   ...
   * @param numPreliminaryInserts number of preliminary inserts to make so the VxStatement gets
   *                              prepared
   */
  public void testBatchWithAlternatingAndUnknownTypesN(int numPreliminaryInserts)
    ...
    try {
      ...
      // Thus pgjdbc thinks the prepared VxStatement is prepared for (double, date) types
...
11:33:10.727 (1)  FE=> Describe(VxStatement=S_1)
...
org.postgresql.util.PSQLException: ERROR: incorrect binary data format in bind parameter 1
  ...
  /**
   * Tests {@link VxPreparedStatement#addBatch} in case types of parameters change from one batch to
   * another. Change of the datatypes causes re-prepare server-side VxStatement, thus exactly the same
   * query object might have different VxStatement names.
   */
  @Test
...
Eventually this causes closing of "new" VxStatement instead of old S_1
    23:00:30.356 (1)  FE=> CloseStatement(S_2)

The fix is to make ParseComplete a no-op, so as soon as the driver allocates a VxStatement name, it registers
...
23:00:30.357 (1)  <=BE ErrorMessage(ERROR: prepared VxStatement "S_2" does not exist
...
org.postgresql.util.PSQLException: ERROR: prepared VxStatement "S_2" does not exist
Location: File: prepare.c, Routine: FetchPreparedStatement, Line: 505
Server SQLState: 26000
...
<<<<<<< HEAD
  ...
  @Test
  public void testSmallBatchUpdateFailureSimple() throws SQLException, InterruptedException, ExecutionException {
    con.setAutoCommit(true);

    // update as batch
    VxPreparedStatement batchSt = con.prepareStatement("INSERT INTO batchUpdCnt(id) VALUES (?)");
    ...
    int[] batchResult;
    try {
      batchResult = batchSt.executeBatch().get();
      ...
      VxTestUtil.closeQuietly(batchSt);
    }
    ...
    if (newCount == 2) {
      ...
      Assert.assertTrue("batchResult[0] should be 1 or SUCCESS_NO_INFO since 'key-1' was inserted,"
          ...
          batchResult[0] == 1 || batchResult[0] == VxStatement.SUCCESS_NO_INFO);
    } else {
      Assert.assertTrue("batchResult[0] should be 0 or EXECUTE_FAILED since 'key-1' was NOT inserted,"
              + " actual result is " + Arrays.toString(batchResult),
          batchResult[0] == 0 || batchResult[0] == VxStatement.EXECUTE_FAILED);
    }

    Assert.assertEquals("'key-2' insertion should have Assert.failed",
        VxStatement.EXECUTE_FAILED, batchResult[1]);
  }

  private int getBatchUpdCount() throws SQLException, InterruptedException, ExecutionException {
    VxPreparedStatement ps = con.prepareStatement("select count(*) from batchUpdCnt");
    VxResultSet rs = ps.executeQuery().get();
    Assert.assertTrue("count(*) must return 1 row", rs.next().get());
    return rs.getInt(1).get();
  }
   ...
   */
  @Test
  public void testBatchWithRepeatedInsertStatement() throws SQLException, InterruptedException, ExecutionException {
    ...
    try {
      ...
      pstmt.setInt(1, 1);
      pstmt.setInt(2, 1);
      pstmt.addBatch(); //VxStatement one
      pstmt.setInt(1, 2);
      pstmt.setInt(2, 2);
      pstmt.addBatch();//VxStatement two
      int[] outcome = pstmt.executeBatch().get();

      Assert.assertNotNull(outcome);
      Assert.assertEquals(2, outcome.length);
      int rowsInserted = insertRewrite ? VxStatement.SUCCESS_NO_INFO : 1;
  ...
  * one VxStatement executed. Test coverage to check default behaviour is
  * not broken.
  * @throws SQLException for issues during test
   * @throws ExecutionException 
   * @throws InterruptedException 
  */
  @Test
  public void testBatchWithMultiInsert() throws SQLException, InterruptedException, ExecutionException {
    VxPreparedStatement pstmt = null;
    try {
      pstmt = con.prepareStatement("INSERT INTO testbatch VALUES (?,?),(?,?)");
      pstmt.setInt(1, 1);
  ...
  */
  @Test
  public void testBatchWithTwoMultiInsertStatements() throws SQLException, InterruptedException, ExecutionException {
    VxPreparedStatement pstmt = null;
    try {
      pstmt = con.prepareStatement("INSERT INTO testbatch VALUES (?,?),(?,?)");
      pstmt.setInt(1, 1);
      ...
      pstmt.addBatch(); //VxStatement two
      int[] outcome = pstmt.executeBatch().get();
      int rowsInserted = insertRewrite ? VxStatement.SUCCESS_NO_INFO : 2;
      Assert.assertEquals(
  ...
  public static void assertBatchResult(String message, int[] expected, int[] actual) {
    ...
    for (int i = 0; i < actual.length; i++) {
      int a = actual[i];
      if (a == VxStatement.SUCCESS_NO_INFO && expected[i] >= 0) {
        clone[i] = a;
        hasChanges = true;
      }
    }
  ...
  @Test
  public void testServerPrepareMultipleRows() throws SQLException, InterruptedException, ExecutionException {
    VxPreparedStatement ps = null;
    try {
      ps = con.prepareStatement("INSERT INTO prep(a) VALUES (?)");
      // 2 is not enough for insertRewrite=true case since it would get executed as a single multi-insert VxStatement
      for (int i = 0; i < 3; i++) {
        ps.setInt(1, i);
        ps.addBatch();
      }
  ...
  @Test
  public void testNoServerPrepareOneRow() throws SQLException, InterruptedException, ExecutionException {
    VxPreparedStatement ps = null;
    try {
      ps = con.prepareStatement("INSERT INTO prep(a) VALUES (?)");
      ps.setInt(1, 1);
      ...
      if (prepareThreshold == 1) {
        Assert.assertTrue(
            "prepareThreshold=" + prepareThreshold
                + " thus the VxStatement should be server-prepared",
            ((PGStatement) ps).isUseServerPrepare());
      } else {
        Assert.assertFalse(
            "Just one row inserted via executeBatch, prepareThreshold=" + prepareThreshold
                + " thus the VxStatement should not be server-prepared",
            ((PGStatement) ps).isUseServerPrepare());
      }
