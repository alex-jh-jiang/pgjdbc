...
public class VxQuotationTest extends VxBaseTest4 {
  ...
  public VxQuotationTest(QuoteStyle quoteStyle, String expected, String expr) {
  ...
  @Test
  public void quotedString() throws SQLException, InterruptedException, ExecutionException {
    VxPreparedStatement ps = con.prepareStatement("select " + expr);
    try {
      VxResultSet rs = ps.executeQuery().get();
      rs.next().get();
      String val = rs.getString(1).get();
  ...
  @Test
  public void bindInTheMiddle() throws SQLException, InterruptedException, ExecutionException {
    VxPreparedStatement ps = con.prepareStatement("select " + expr + ", ?, " + expr);
    try {
      ps.setInt(1, 42);
      VxResultSet rs = ps.executeQuery().get();
      rs.next().get();
      String val1 = rs.getString(1).get();
      String val3 = rs.getString(3).get();
