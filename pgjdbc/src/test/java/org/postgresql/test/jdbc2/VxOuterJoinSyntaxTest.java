 ...
 */
public class VxOuterJoinSyntaxTest extends VxBaseTest4 {
  ...
  private void testOuterJoinSyntax(String theQuery, List<String> expectedResult) throws Exception {
    ...
    try {
      final VxResultSet rs = st.executeQuery(theQuery).get();
      try {
        Assert.assertEquals("SQL " + theQuery, VxTestUtil.join(VxTestUtil.resultSetToLines(rs).get()), VxTestUtil.join(expectedResult));
      } finally {
