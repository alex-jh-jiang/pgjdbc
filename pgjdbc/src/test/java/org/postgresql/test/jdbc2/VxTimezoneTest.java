...
public class VxTimezoneTest {
  ...
  public void setUp() throws Exception {
    ...
    connect();
    VxTestUtil.createTable(con, "testtimezone",
    ...
    con.createStatement().executeUpdate("set timezone = 'gmt-3'").get();
  ...
  public void tearDown() throws Exception {
    ...
    VxTestUtil.dropTable(con, "testtimezone");
  ...
  public void testGetTimestamp() throws Exception {
    ...
    for (int i = 0; i < PREPARE_THRESHOLD; i++) {
      ...
      VxPreparedStatement ps = con.prepareStatement("SELECT tstz,ts,t,tz,d from testtimezone");
  ...
  public void testGetDate() throws Exception {
    ...
    for (int i = 0; i < PREPARE_THRESHOLD; i++) {
      ...
      // timestamptz: 2005-01-01 15:00:00+03
      d = rs.getDate(1).get(); // 2005-01-01 13:00:00 +0100 -> 2005-01-01 00:00:00 +0100
      assertEquals(1104534000000L, d.getTime());
      d = rs.getDate(1, cUTC).get(); // 2005-01-01 12:00:00 +0000 -> 2005-01-01 00:00:00 +0000
      assertEquals(1104537600000L, d.getTime());
      d = rs.getDate(1, cGMT03).get(); // 2005-01-01 15:00:00 +0300 -> 2005-01-01 00:00:00 +0300
      assertEquals(1104526800000L, d.getTime());
      d = rs.getDate(1, cGMT05).get(); // 2005-01-01 07:00:00 -0500 -> 2005-01-01 00:00:00 -0500
      assertEquals(1104555600000L, d.getTime());
      d = rs.getDate(1, cGMT13).get(); // 2005-01-02 01:00:00 +1300 -> 2005-01-02 00:00:00 +1300
      assertEquals(1104577200000L, d.getTime());

      // timestamp: 2005-01-01 15:00:00
      d = rs.getDate(2).get(); // 2005-01-01 00:00:00 +0100
      assertEquals(1104534000000L, d.getTime());
      d = rs.getDate(2, cUTC).get(); // 2005-01-01 00:00:00 +0000
      assertEquals(1104537600000L, d.getTime());
      d = rs.getDate(2, cGMT03).get(); // 2005-01-01 00:00:00 +0300
      assertEquals(1104526800000L, d.getTime());
      d = rs.getDate(2, cGMT05).get(); // 2005-01-01 00:00:00 -0500
      assertEquals(1104555600000L, d.getTime());
      d = rs.getDate(2, cGMT13).get(); // 2005-01-01 00:00:00 +1300
      assertEquals(1104490800000L, d.getTime());

      // date: 2005-01-01
      d = rs.getDate(3).get(); // 2005-01-01 00:00:00 +0100
      assertEquals(1104534000000L, d.getTime());
      d = rs.getDate(3, cUTC).get(); // 2005-01-01 00:00:00 +0000
      assertEquals(1104537600000L, d.getTime());
      d = rs.getDate(3, cGMT03).get(); // 2005-01-01 00:00:00 +0300
      assertEquals(1104526800000L, d.getTime());
      d = rs.getDate(3, cGMT05).get(); // 2005-01-01 00:00:00 -0500
      assertEquals(1104555600000L, d.getTime());
      d = rs.getDate(3, cGMT13).get(); // 2005-01-01 00:00:00 +1300
      assertEquals(1104490800000L, d.getTime());
  ...
  public void testSetTimestampOnTime() throws Exception {
    ...
    for (int i = 0; i < PREPARE_THRESHOLD; i++) {
      ...
      seq = 1;
      VxPreparedStatement ps = con.prepareStatement("SELECT seq,t FROM testtimezone ORDER BY seq");
      VxResultSet rs = ps.executeQuery().get();
  ...
  public void testSetDate() throws Exception {
    for (int i = 0; i < PREPARE_THRESHOLD; i++) {
      ...
      assertTrue(rs.next().get());
      assertEquals(seq++, rs.getInt(1));
      assertEquals(dJVM, rs.getDate(2).get());
  ...
  public void testSetTime() throws Exception {
    for (int i = 0; i < PREPARE_THRESHOLD; i++) {
      ...
      seq = 1;
      VxPreparedStatement ps = con.prepareStatement("SELECT seq,t,tz FROM testtimezone ORDER BY seq");
      VxResultSet rs = ps.executeQuery().get();
  ...
  public void testHalfHourTimezone() throws Exception {
    ...
    for (int i = 0; i < PREPARE_THRESHOLD; i++) {
      ...
      VxResultSet rs = ps.executeQuery().get();
      assertTrue(rs.next().get());
      assertEquals(0L, rs.getTimestamp(1).get().getTime());
      ps.close();
    }
  }
  ...
  public void testTimezoneWithSeconds() throws SQLException, InterruptedException, ExecutionException {
    ...
    for (int i = 0; i < PREPARE_THRESHOLD; i++) {
      ...
      assertEquals(-1577923200000L, rs.getTimestamp(1).get().getTime());
      ps.close();
    }
  }
  ...
  public void localTimestamps(String timeZone) throws Exception {
    ...
    for (int i = 0; i < PREPARE_THRESHOLD; i++) {
      ...
      for (int j = 0; rs.next().get(); j++) {
        String testDate = datesToTest.get(j);
        Date getDate = rs.getDate(1).get();
        Date getDateFromDateColumn = rs.getDate(2).get();
        Timestamp getTimestamp = rs.getTimestamp(1).get();
        String getString = rs.getString(1).get();
        Time getTime = rs.getTime(1).get();
  ...
  private void checkDatabaseContents(String query, String[][] correct) throws Exception {
    ...
    VxStatement s = con2.createStatement();
    assertFalse(s.execute("set time zone 'UTC'").get());
    assertTrue(s.execute(query).get());
    VxResultSet rs = s.getResultSet();
    for (int j = 0; j < correct.length; ++j) {
      assertTrue(rs.next().get());
      for (int i = 0; i < correct[j].length; ++i) {
        assertEquals("On row " + (j + 1), correct[j][i], rs.getString(i + 1).get());
      }
    }
