...
public class VxBatchFailureTest extends VxBaseTest4 {
  ...
  enum BatchType {
    SIMPLE {
      @Override
      public VxStatement createStatement(VxConnection con) throws SQLException {
    ...
    public abstract VxStatement createStatement(VxConnection con) throws SQLException;

    public void addRow(VxStatement VxStatement, String value) throws SQLException {
      switch (this) {
        case SIMPLE:
          VxStatement.addBatch("INSERT INTO batchUpdCnt(id) VALUES ('" + value + "')");
          break;
        case PREPARED:
        case PREPARED_WITH_GENERATED:
          VxPreparedStatement ps = (VxPreparedStatement) VxStatement;
  ...
  private enum FailMode {
    ...
    public void injectFailure(VxStatement statement, BatchType batchType) throws SQLException {
      switch (this) {
  ...
  public VxBatchFailureTest(BatchType batchType, AutoCommit autoCommit,
  ...
  public void run() throws SQLException, InterruptedException, ExecutionException {
    VxStatement statement = batchType.createStatement(con);
    ...
    int expectedRows = 1;
    try {
      batchResult = statement.executeBatch().get();
      ...
      if (batchType == BatchType.PREPARED_WITH_GENERATED) {
        VxResultSet rs = statement.getGeneratedKeys();
        while (rs.next().get()) {
          keys.add(rs.getString(1).get());
        }
      }
      statement.close();
    }
