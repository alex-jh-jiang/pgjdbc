...
import org.postgresql.util.VxJdbcBlackHole;
...
public class VxDatabaseMetaData {
  ...
  public VxDatabaseMetaData(VxConnection conn) {
    this.connection = conn;
  }

  private String keywords;

  protected final VxConnection connection; // The connection association
  ...
  public VxResultSet getProcedureColumns(String catalog, String schemaPattern,
    ...
    while (rs.next().get()) {
      ...
      byte[] specificName =
                connection.encodeString(rs.getString("proname").get() + "_" + rs.getString("oid").get());
      int returnType = (int) (rs.getLong("prorettype").get().intValue());
      String returnTypeType = rs.getString("typtype").get();
      int returnTypeRelid = (int) rs.getLong("typrelid").get().intValue();

      String strArgTypes = rs.getString("proargtypes").get();
      ...
      Array argNamesArray = rs.getArray("proargnames").get();
      if (argNamesArray != null) {
        argNames = (String[]) argNamesArray.getArray();
      }

      String[] argModes = null;
      Array argModesArray = rs.getArray("proargmodes").get();
      ...
      Array allArgTypesArray = rs.getArray("proallargtypes").get();
      ...
      for (int i = 0; i < numArgs; i++) {
        ...
        int columnMode = java.sql.DatabaseMetaData.procedureColumnIn;
        if (argModes != null && argModes[i].equals("o")) {
          columnMode = java.sql.DatabaseMetaData.procedureColumnOut;
        } else if (argModes != null && argModes[i].equals("b")) {
          columnMode = java.sql.DatabaseMetaData.procedureColumnInOut;
        } else if (argModes != null && argModes[i].equals("t")) {
          columnMode = java.sql.DatabaseMetaData.procedureColumnReturn;
        }
      ...
      if (returnTypeType.equals("c") || (returnTypeType.equals("p") && argModesArray != null)) {
        ...
        VxStatement columnstmt = connection.createStatement();
        VxResultSet columnrs = columnstmt.executeQuery(columnsql).get();
        while (columnrs.next().get()) {
          int columnTypeOid = (int) columnrs.getLong("atttypid").get().intValue();
          byte[][] tuple = new byte[columns][];
          tuple[0] = null;
          tuple[1] = schema;
          tuple[2] = procedureName;
   ...
   */
  public VxResultSet getCatalogs() throws SQLException {
    Field[] f = new Field[1];
  ...
  public VxResultSet getTableTypes() throws SQLException {
  ...
  public VxResultSet getColumns(String catalog, String schemaPattern, String tableNamePattern,
    ...
    while (rs.next().get()) {
      ...
      String defval = rs.getString("adsrc").get();
      ...
      String identity = rs.getString("attidentity").get();
      ...
      tuple[10] = connection.encodeString(Integer.toString(rs.getBoolean("attnotnull").get()
      ...
      // Is nullable
      tuple[17] = connection.encodeString(rs.getBoolean("attnotnull").get() ? "NO" : "YES");

      int baseTypeOid = (int) rs.getLong("typbasetype").get().intValue();
   ...
   * @throws InterruptedException 
   */
  protected VxResultSet getImportedExportedKeys(String primaryCatalog, String primarySchema,
    ...
    String sql =
        "SELECT NULL::text AS PKTABLE_CAT, pkn.nspname AS PKTABLE_SCHEM, pkc.relname AS PKTABLE_NAME, pka.attname AS PKCOLUMN_NAME, "
            ...
            + " WHEN 'c' THEN " + java.sql.DatabaseMetaData.importedKeyCascade
            + " WHEN 'n' THEN " + java.sql.DatabaseMetaData.importedKeySetNull
            + " WHEN 'd' THEN " + java.sql.DatabaseMetaData.importedKeySetDefault
            + " WHEN 'r' THEN " + java.sql.DatabaseMetaData.importedKeyRestrict
            + " WHEN 'p' THEN " + java.sql.DatabaseMetaData.importedKeyRestrict
            + " WHEN 'a' THEN " + java.sql.DatabaseMetaData.importedKeyNoAction
            ...
            + " WHEN con.condeferrable THEN " + java.sql.DatabaseMetaData.importedKeyInitiallyImmediate
            + " ELSE " + java.sql.DatabaseMetaData.importedKeyNotDeferrable
  ...
  public VxResultSet getImportedKeys(String catalog, String schema, String table)
      throws SQLException, InterruptedException, ExecutionException {
    return getImportedExportedKeys(null, null, null, catalog, schema, table);
  }

  public VxResultSet getExportedKeys(String catalog, String schema, String table)
      throws SQLException, InterruptedException, ExecutionException {
    return getImportedExportedKeys(catalog, schema, table, null, null, null);
  }

  public VxResultSet getCrossReference(String primaryCatalog, String primarySchema,
      String primaryTable, String foreignCatalog, String foreignSchema, String foreignTable)
          throws SQLException, InterruptedException, ExecutionException {
  ...
  public VxResultSet getTypeInfo() throws SQLException, InterruptedException, ExecutionException {
    ...
    while (rs.next().get()) {
      ...
      String typname = rs.getString(1).get();
      int typeOid = (int) rs.getLong(2).get().intValue();
  ...
  public VxResultSet getIndexInfo(String catalog, String schema, String tableName,
      boolean unique, boolean approximate) throws SQLException, InterruptedException, ExecutionException {
    /*
  ...
  protected VxStatement createMetaDataStatement() throws SQLException {
  ...
  public VxResultSet getFunctions(String catalog, String schemaPattern, String functionNamePattern)
      throws SQLException, InterruptedException, ExecutionException {
    ...
    String funcTypeSql = java.sql.DatabaseMetaData.functionResultUnknown + " ";
    if (pgFuncResultExists) {
      funcTypeSql = " CASE "
              + "   WHEN (format_type(p.prorettype, null) = 'unknown') THEN " + java.sql.DatabaseMetaData.functionResultUnknown
              + "   WHEN "
              + "     (substring(pg_get_function_result(p.oid) from 0 for 6) = 'TABLE') OR "
              + "     (substring(pg_get_function_result(p.oid) from 0 for 6) = 'SETOF') THEN " + java.sql.DatabaseMetaData.functionReturnsTable
              + "   ELSE " + java.sql.DatabaseMetaData.functionNoTable
              + " END ";
    }
  ...
  public VxResultSet getPseudoColumns(String catalog, String schemaPattern, String tableNamePattern,
  ...
  public VxResultSet getSuperTypes(String catalog, String schemaPattern, String typeNamePattern)
      throws SQLException {
    throw org.postgresql.Driver.notImplemented(this.getClass(),
        "getSuperTypes(String,String,String)");
  }

  public VxResultSet getSuperTables(String catalog, String schemaPattern, String tableNamePattern)
      throws SQLException {
    throw org.postgresql.Driver.notImplemented(this.getClass(),
        "getSuperTables(String,String,String,String)");
  }

  public VxResultSet getAttributes(String catalog, String schemaPattern, String typeNamePattern,
